...
@st.cache_data
def load_data():
    df = pd.read_csv("Data_Performa_Karyawan.csv")
    return df
...
st.subheader("1. Dataset Karyawan")
st.dataframe(df, width="stretch", height=400)

st.subheader("2. Alternatif & Kriteria")
alt_col = "Nama_Karyawan"
...
if st.session_state.step == 3:
    st.warning("Lanjutkan untuk proses penentuan performa karyawan terbaik?")
...
if st.session_state.step == 6:
    st.header("Tentukan Jenis Kriteria Karyawan Terbaik")
    impacts = []
